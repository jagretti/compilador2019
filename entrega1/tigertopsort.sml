	...
	fun candidato E P =
        ...
        fun tsort P [] Res = rev Res
          ...
          | tsort P (St as (h::t)) Res =
            let
		val x = (hd(candidato St P)) handle Empty => raise Ciclo
            in
	    ...
	    List.foldr (fn((x, y), l) =>
			   let
			       val l1 = case List.find (op= rs x) l of
                                            NONE => x::l
					  | _ => l
  ...
  | procesa (sorted as (h::t)) (pares:{name:symbol, ty:ty} list) env recs =
    let
        ...
        val ttopt = case tabBusca(h, env) of
			    ...
			    SOME (n, tr) => (tabRInserta(h, tr, env); SOME tr) (* OJOOOOOOOOOOOOOOOO *)
...
fun fijaNONE [] env = env
  ...
  | fijaNONE((name, TRecord(lf, u))::t) env =
    let
	fun busNONE(s, ar as (ref(TTipo t)), _) =
